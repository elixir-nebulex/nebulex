...
ok/error tuple API). Previously, it did return `nil` when the given key wasn't
in the cache. Now, the callback accepts an argument to specify the default value
...
the `:default_key_generator` option should be provided to `use Nebulex.Caching`.
...
[nbx_caching]: https://hexdocs.pm/nebulex/Nebulex.Caching.html#module-compilation-time-options
